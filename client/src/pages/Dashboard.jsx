          ...
          <div className="w-full flex flex-row items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 sm:gap-5 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <h1 className="text-sm sm:text-2xl font-black tracking-wide uppercase text-zinc-100 leading-tight truncate">
                  {user ? user.username : "Loading..."}
                </h1>
                <div className="mt-0.5 sm:mt-1">
                  <span className="bg-yellow-400/20 text-yellow-400 border border-yellow-400/40 text-[8px] sm:text-[10px] font-black px-1.5 sm:px-3 py-0.5 rounded tracking-widest uppercase inline-block">
                    VIP
                  </span>
                </div>
              </div>
            ...
            <div key={game.id} className="group bg-zinc-900/90 border-2 border-zinc-800/80 hover:border-yellow-400 rounded-3xl p-4 sm:p-5 flex flex-col justify-between shadow-2xl transition-all duration-300 backdrop-blur-sm">
              <div>
                <div className="overflow-hidden rounded-2xl mb-4 h-40 sm:h-44 w-full bg-zinc-950 relative border border-zinc-800/50">
                  <img src={game.img} alt={game.title} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-85" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1596838132731-3301c3fd4317?q=80&w=600"; }} />
                  {game.type === "free" && (
                    <span className="absolute top-3 left-3 sm:top-4 sm:left-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-[9px] sm:text-[10px] font-black px-2 sm:px-2.5 py-0.5 rounded shadow-lg uppercase tracking-widest">Free Entry</span>
                  )}
                </div>
                <div className="w-full px-1 sm:px-3">
                  <h2 className="text-zinc-100 text-lg sm:text-xl font-black tracking-wide mb-1 group-hover:text-yellow-400 transition-colors">{game.title}</h2>
                  <p className="text-zinc-400 text-[11px] sm:text-xs leading-relaxed mb-4 min-h-[32px]">{game.desc}</p>
                </div>
              </div>
              <div className="w-full px-1 pb-1">
                {game.type === "free" ? (
                  <button onClick={() => navigate("/free-game")} className="w-full bg-gradient-to-r from-yellow-500 via-amber-400 to-yellow-500 text-black rounded-xl text-xs sm:text-sm font-black tracking-widest py-3 hover:brightness-110 active:scale-95 transition-all">FREE PLAY</button>
                ) : (
                  <button onClick={() => alert("Level low. Increase your level by playing Free Game")} className="w-full bg-zinc-950/90 text-yellow-400/70 border-2 border-zinc-800 py-3 rounded-xl text-xs sm:text-sm font-black tracking-widest">LOCK PLAY</button>
                )}
              </div>
            </div>
          ))}
